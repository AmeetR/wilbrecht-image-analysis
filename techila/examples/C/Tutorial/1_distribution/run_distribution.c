...
int main(int argc, char** argv) {
  ...
  int jobs = 5;   // Will define the number of Jobs in the Project
  ...
  techila_peach_setName(ph, "Distribution_example");
  ...
  if(access("distribution_dist32.exe", F_OK ) != -1 ) {
    techila_peach_addExeFile(ph, "exe", "distribution_dist32.exe", "Windows", "x86");
  }
  if(access("distribution_dist32", F_OK ) != -1 ) {
    techila_peach_addExeFile(ph, "exe", "distribution_dist32", "Linux", "i386");
  }
  if(access("distribution_dist64", F_OK ) != -1 ) {
    techila_peach_addExeFile(ph, "exe", "distribution_dist64", "Linux", "amd64");
  }

  // Enable messages
  techila_peach_setMessages(ph,1);

  // Set the number of jobs to 5 (value of the 'jobs' variable)
  techila_peach_setJobs(ph, jobs);

  // Define the name of the output file that will be returned from the Workers
  ...
  while (len > 0) { // Do while more result files are available
    ...
    checkStatus(status); // Check the status code
    if (len > 0) { // Do if a valid result file is available
      file[len] = '\0';
      int res = handle_result_file(file); // Process the result file
      printf("Job %i, result %i\n", i + 1, res); // Display the result
      myarray[i] = res; // Store the result in an array
      i++;
    }
  }
