# Copyright 2017 QuantBros
# Copyright 2017 Techila Technologies Ltd.
#
# Please see following tutorial video for a walkthrough of this code:
# https://www.youtube.com/watch?v=n75pYmmBmo4
...
tickerData <- llply(Tickers,
                    function(ticker, sDate, eDate) {
                        getSymbols(ticker,
                                   src = "yahoo",
                                   from = sDate,
                                   to = eDate,
                    ...
                    sDate=sDate,
                    eDate=eDate,
                    .paropts = list(.options.packages = list("quantmod", "curl")))
...
charts.PerformanceSummary(stockReturns,
                          main = "Individual Stock Cumulative Performance",
                          geometric = T)
...
plotPrices <- function(preresult){
    invisible(lines(as.zoo(cumprod(1 + preresult[[1]]) - 1),
                    col = adjustcolor("black", alpha.f = 0.1))) # Realtime Charting
    return(preresult)
}
...
# Generate base plot
plot(as.zoo(cumprod(1 + test) - 1),
     ylim = c(-1.0, 10),
     ylab = "Cumulative Return",
     col = adjustcolor("black", alpha.f = 0.0))

# Now generate portfolios and dynamically generate the chart, capture
# the pre-results
...
charts.PerformanceSummary(result.zoo[, 1:10],
                          main = "Cumulative Performance of Some Simulations",
                          geometric = F)
...
for(i in 1:length(maxWeights)){
    ...
    # Calculate the average path across each simulation for this
    # weight parameter
    path.monthly.df$Average <- apply(path.monthly.df[, 1:numSims], 1, mean)

    # Return all values to the melted object
    meltedPath <- rbind(meltedPath,
                        melt(path.monthly.df,
                             id.vars = c("Date", "MaxWeight", "Average"),
                             variable.name = "Seed",
                             value.name = "Return"))
}
...
for(i in 1:length(maxWeights)){
    ...
    metricsDF <- data.frame(Return = Return,
                            Risk = Risk,
                            Sharpe = Sharpe,
