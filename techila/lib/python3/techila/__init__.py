...
class ResultIter(object):
    ...
    def __next__(self):
        ...
        filename = ctypes.create_string_buffer(b'\000', size=255)
        ...
        iszip = False
        filenamevalue = bytes.decode(filename.value)
        filelist = [filenamevalue]
        unzipdir = None
        ...
        if self.useoutputfiles:
            ...
            import zipfile
            zf = zipfile.ZipFile(filenamevalue)
...
def describeStatusCode(code):
    ...
    if Techila._libloaded:
        errorstr = ctypes.create_string_buffer(b'\000', size=255)
        length = ctypes.c_int(255)
...
def init(sdkroot = None,
    ...
    if not sdkroot:
        ...
        if envroot is not None:
            ...
            try:
                import techila.sdkroot_config
                sdkroot = techila.sdkroot_config.sdkroot
            except ImportError:
    ...
    if not Techila._initialized:
        sc = Techila._lib.techila_initFile(_tb(initfile))
        if sc != 0:
...
def getTempDir():
    ...
    tdir = None
    if Techila._initialized:
        dir0 = ctypes.create_string_buffer(b'\000', size=255)
        length = ctypes.c_int(255)
        Techila._lib.techila_getTempDir(dir0, ctypes.byref(length))

        if length.value > 0:
            tdir = bytes.decode(dir0.value)
...
def peach(funcname = None,
    ...
    if type(funcname).__name__ == 'function':
        resumefn = 'func_' + funcname.__name__
    ...
    if resume and os.path.exists(resumefn):
        ...
        resumedata = pickle.Unpickler(resumefile)
    ...
    envver3 = os.environ.get('TECHILA_PYTHON3_VERSION')
    if python_version is None:
        if envver3 is not None:
            python_version = envver3
        elif envver is not None:
    ...
    if projectid is not None and projectid > 0:
        ...
        lib.techila_peach_setResultFile(ph, _tb(resultfile))
    else:
        # execute
        ...
        if python_required:
            ...
            if run_as_python:
                if peachclient:
                    lib.techila_peach_setExecutable(ph, _tb('%A(python)'))
                lib.techila_peach_putProjectParam(ph, _tb('techila_worker_features'), _tb('(python=*)'))
            else:

                # Set peach parameters
                if peachclient:
                    lib.techila_peach_setExecutable(ph, _tb('%L(python)'))
                ...
                lib.techila_peach_addExeImport(ph, _tb(import_name))
                lib.techila_peach_putExeExtras(ph, _tb('ExternalResources'), _tb('python;resource=python;file=python.exe;osname=Windows,python;resource=python;file=python;osname=Linux,python;resource=python;file=python;osname=Mac OS X,pythondir;resource=python'))
                envadd = '%C(path.separator,PROPGET)%L(pythondir)%C(file.separator,PROPGET)Lib%C(path.separator,PROPGET)%L(pythondir)%C(file.separator,PROPGET)Lib%C(file.separator,PROPGET)site-packages%C(path.separator,PROPGET)%L(pythondir)%C(file.separator,PROPGET)Lib%C(file.separator,PROPGET)dist-packages%C(path.separator,PROPGET)%L(pythondir)%C(file.separator,PROPGET)Lib%C(file.separator,PROPGET)lib-dynload%C(path.separator,PROPGET)%L(pythondir)%C(file.separator,PROPGET)Modules'
            ...
            lib.techila_peach_putExeExtras(ph, _tb('Environment'), _tb(envstr))

        if realexecutable:
            lib.techila_peach_setExecutable(ph, _tb(realexecutable))
        ...
        if outputfiles:
            for of in outputfiles:
                lib.techila_peach_addOutputFile(ph, _tb(of))
        ...
        if binaries:
            for bin in binaries:
                ...
                lib.techila_peach_addExeFile(ph, _tb(binres), _tb(binfile), _tb(binos), _tb(binprocessor))
        ...
        if files:
            ...
            for filename in files:
                lib.techila_peach_addExeFile(ph, _tb(os.path.basename(filename)), _tb(filename), None, None)
        ...
        if peachclient:
            ...
            lib.techila_peach_addExeFile(ph, _tb('peachclient.py'), _tb(peachclientfile), None, None)
            ...
            lib.techila_peach_addOutputFile(ph, _tb('peachoutput;file=techila_peach_result'))

        if exeparams is not None:
            lib.techila_peach_putExeExtras(ph, _tb('Parameters'), _tb(exeparams))


        if snapshot:
            lib.techila_peach_putExeExtras(ph, _tb('SnapshotFiles'), _tb(snapshotfiles))
            lib.techila_peach_putExeExtras(ph, _tb('SnapshotInterval'), _tb(str(snapshotinterval)))
        ...
        if binary_bundle_parameters:
            ...
            for name in list(binary_bundle_parameters.keys()):
                lib.techila_peach_putExeExtras(ph, _tb(name), _tb(str(binary_bundle_parameters.get(name))))
        ...
        if databundles:
            for databundle in databundles:
                ...
                for dbdatafile in dbdatafiles:
                    lib.techila_peach_addDataFileWithDir(ph, _tb(dbdatadir), _tb(dbdatafile))
                ...
                if dbparams:
                    ...
                    for name in list(dbparams.keys()):
                        lib.techila_peach_putDataExtras(ph, _tb(name), _tb(str(dbparams.get(name))))
        ...
        if jobinputfiles:
            ...
            for jidatafile in jidatafiles:
                if type(jidatafile) is str:
                    jidatafile = [_tb(jidatafile)]

                jilen = len(jidatafile)
                cjidf = ctypes.c_char_p * jilen
                jidf = cjidf(*jidatafile)

                lib.techila_peach_addJobFileWithDir3(ph, _tb(jidatadir), jilen, jidf)
            ...
            if jifilenames:
                ...
                jifilenames = [_tb(f) for f in jifilenames]
            ...
            if jiparams:
                ...
                for name in list(jiparams.keys()):
                    lib.techila_peach_putJobExtras(ph, _tb(name), _tb(str(jiparams.get(name))))
        ...
        if datafiles:
            ...
            for datafile in datafiles:
                lib.techila_peach_addParamFile(ph, _tb(datafile))
        ...
        if peachclient:
            ...
            data = pickle.Pickler(file, 1)
            data.dump(mode)

            funcdata = []
            if mode == 'function':
                code_str = marshal.dumps(function.__code__)
                defaults = function.__defaults__
                closure_values = []
                if function.__closure__:
                    closure_values = [v.cell_contents for v in function.__closure__]
            ...
            if funclist is not None:
                ...
                for xfunc in funclist:
                    ...
                    if type(xfunc).__name__ == 'function':
                        xmode = 'function'
                        xfunction = xfunc
                        xfuncname = xfunc.__name__

                        xfuncdata.append(xmode)

                        xcode_str = marshal.dumps(xfunction.__code__)
                        xdefaults = xfunction.__defaults__
                        xclosure_values = []
                        if xfunction.__closure__:
                            xclosure_values = [v.cell_contents for v in xfunction.__closure__]
            ...
            lib.techila_peach_addParamFileWithTarget(ph, _tb(inputdataname), _tb('techila_peach_inputdata'))
        ...
        if bundle_parameters:
            ...
            for name in list(bundle_parameters.keys()):
                lib.techila_peach_putParamExtras(ph, _tb(name), _tb(str(bundle_parameters.get(name))))
        ...
        if project_parameters:
            ...
            for name in list(project_parameters.keys()):
                lib.techila_peach_putProjectParam(ph, _tb(name), _tb(str(project_parameters.get(name))))
        ...
        if description:
            lib.techila_peach_setDescription(ph, _tb(description))
        ...
        if resume:
            ...
            resumedata = pickle.Pickler(resumefile, 1)
    ...
    if executesc != 0:
        msg = ctypes.create_string_buffer(b'\000', size=4096)
...
def create_signed_bundle(name,
    ...
    for key in list(files.keys()):
        filesarray.append(_tb(key + '=' + files.get(key)))

    extrasarray = []
    if extras is not None:
        for key in list(extras.keys()):
            extrasarray.append(_tb(key + '=' + extras.get(key)))
    ...
    try:
        ...
        exists = lib.techila_bundleVersionExists(_tb(name), _tb(version))

        if exists:
            raise TechilaException('Bundle already exists', -1, name)

        ret = lib.techila_createSignedBundle3(handle, _tb(name), _tb(description), _tb(version), _tb(imports), _tb(exports), _tb(natives), _tb(category), _tb(resource), _tb(''), _tb(''), _tb(''), len(extrasarray), len(filesarray), ca)

        if ret != 0:
            raise TechilaException('Failed creating bundle', ret)

        cbundlename = ctypes.create_string_buffer(b'\000', size=1024)
        cbnlen = ctypes.c_int(1024)
        lib.techila_getLastCreatedBundleName(handle, cbundlename, ctypes.byref(cbnlen))

        tmpbundlename = bytes.decode(cbundlename.value)

        #print tmpbundlename

        exists = lib.techila_bundleVersionExists(_tb(tmpbundlename), _tb(version))
...
def get_java_system_property(name):
    ...
    if Techila._libloaded:
        ...
        value = ctypes.create_string_buffer(b'\000', size=1024)
        length = ctypes.c_int(1024)
        r = Techila._lib.techila_getJavaSystemProperty(_tb(name), value, ctypes.byref(length))

        #print r

        rvalue = bytes.decode(value.value)
...
def send_im_data(jobid, data, handle = -1):
    if Techila._libloaded:
        ...
        try:
            if size < 256 * 1024:
                b64data = bytes.decode(base64.encodebytes(pickle.dumps(data)))
            ...
            exitcode = lib.techila_sendJobPostParameter(handle, ljobid, _tb(command))
