...
class TechilaSocket(object):
    ...
    def __init__(self):
        fulljobid = int(os.getenv('TECHILA_JOBID'))
...
class TechilaSemaphore(object):
    ...
    SEMAPHORELOCALREQ = 0x0501
    SEMAPHORELOCALREL = 0x0502
    SEMAPHOREGLOBALREQ = 0x0503
    SEMAPHOREGLOBALREL = 0x0504
    SEMAPHORELOCALREQTO = 0x0505
    SEMAPHOREGLOBALREQTO = 0x0506
    ...
    def reserve(self):
        ...
        if isglobal:
            if timeout < 0:
                m = struct.pack('!hh', TechilaSemaphore.SEMAPHOREGLOBALREQ, namelen) + bytes(name, 'ASCII')
            else:
                m = struct.pack('!hih', TechilaSemaphore.SEMAPHOREGLOBALREQTO, timeout, namelen) +  bytes(name, 'ASCII')
        else:
            if timeout < 0:
                m = struct.pack('!hh', TechilaSemaphore.SEMAPHORELOCALREQ, namelen) +  bytes(name, 'ASCII')
            else:
                m = struct.pack('!hih', TechilaSemaphore.SEMAPHORELOCALREQTO, timeout, namelen) +  bytes(name, 'ASCII')
    ...
    def release(self):
        ...
        namelen = len(name)
        if isglobal:
            m = struct.pack('!hh', TechilaSemaphore.SEMAPHOREGLOBALREL, namelen) +  bytes(name, 'ASCII')
        else:
            m = struct.pack('!hh', TechilaSemaphore.SEMAPHORELOCALREL, namelen) +  bytes(name, 'ASCII')
...
class TechilaInterconnect(object):
    ...
    def cloudop(self, op, data, target = 0):
        ...
        if jobid > 1:
            self.send_data_to_job(jobid >> 1, data)
            if target == 0:
                result = self.recv_data_from_job(jobid >> 1)
            elif target == jobid:
...
def send_im_data(imdata):
    ...
    m = struct.pack('!hh', TechilaInterconnect.SENDIMDATAFILE, l) + bytes(imdatafile, 'ASCII')
